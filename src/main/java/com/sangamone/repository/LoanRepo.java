...
public interface LoanRepo extends JpaRepository<Loan, Integer>{
	...
	List<Loan> viewLoanDetailsByUserId(@Param("from_user_id") int from_user_id,@Param("to_user_id") int to_user_id);

	@Query(value="select * from loan where center_id=:center_id",nativeQuery=true)
	List<Loan> viewLoanDetailsByCenterId(@Param("center_id") int center_id);
